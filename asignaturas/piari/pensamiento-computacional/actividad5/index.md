...
> - **Niveles avanzados (36 – 61):** Estructuras de repetición con bucles `while`, combinadas con condicionales anidados y operadores lógicos.
> - **Nivel 62:** Reto final de maestría para completar el juego al 100%.
{: .alert-tip}
...
## 📊 Rúbrica – Actividad 5: ¿Cómo de lejos llegarás en Compute IT? (máx. 10 puntos)

| Criterio | 0 puntos | 1 punto | 2 puntos | 3 puntos |
|----------|----------|----------|----------|----------|
| **Lectura de secuencias y condicionales (Niveles 1 a 35)** | No supera los niveles iniciales básicos (menos de 10 niveles). | Interpreta instrucciones secuenciales directas y condiciones simples `if` (alcanza entre el nivel 10 y el 20). | Interpreta con soltura estructuras condicionales `if` / `else` según colores o estados (alcanza entre el nivel 21 y el 30). | Domina bifurcaciones condicionales múltiples con `if`, `elif` y `else` (alcanza el nivel 35). |
| **Lectura de bucles while y lógica compleja (Niveles 36 a 61)** | No alcanza los niveles con bucles `while` (menos de 36 niveles). | Interpreta y ejecuta bucles condicionales `while` sencillos en tiempo real (alcanza entre el nivel 36 y el 45). | Resuelve retos que combinan bucles `while` con estructuras condicionales `if`/`elif`/`else` anidadas (alcanza entre el nivel 46 y el 55). | Domina la lógica compleja y la toma de decisiones rápida con condiciones compuestas (alcanza entre el nivel 56 y el 61). |
| **Superación completa del juego (Reto de 62 niveles)** | No completa los 62 niveles del juego (se queda en el nivel 61 o inferior). | Supera con éxito **todos los niveles (los 62 niveles completos)** de Compute IT. | – | – |
| **Presentación de la captura e identificación** | Sin captura de pantalla, ilegible o no corresponde al juego. | Captura de pantalla nítida y completa donde se distingue el nivel alcanzado y figura su **nombre y apellidos** rotulado. | – | – |
| **Entrega en plazo** | Entrega tarde sin justificación o no entrega. | Entrega con pequeño retraso (hasta 2 días). | Entrega puntual dentro del plazo establecido. | – |
